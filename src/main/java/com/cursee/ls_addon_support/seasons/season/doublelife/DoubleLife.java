...
public class DoubleLife extends Season {
  ...
  public static final RegistryKey<DamageType> SOULMATE_DAMAGE = RegistryKey.of(
      RegistryKeys.DAMAGE_TYPE, Identifier.of(
          LSAddonSupport.MOD_ID, "soulmate"));
  ...
  public SessionAction actionRandomTP = new SessionAction(5,
      "§7Random teleport distribution §f[00:00:01]", "Random teleport distribution") {
    @Override
    public void trigger() {
      distributePlayers();
    }
  ...
  public StringListConfig getSoulmateConfig() {
    return new StringListConfig("./config/lifeseries/main",
        "DO_NOT_MODIFY_doublelife_soulmates.properties");
  }
  ...
  public void onPlayerRespawn(ServerPlayerEntity player) {
    ...
    ServerPlayerEntity soulmate = getSoulmate(player);
      if (soulmate != null) {
          syncPlayerInventory(soulmate, player);
      }
  }
  ...
  public void updateOrderedSoulmates() {
    ...
    for (Map.Entry<UUID, UUID> entry : soulmates.entrySet()) {
        if (soulmatesOrdered.containsKey(entry.getKey()) || soulmatesOrdered.containsValue(
            entry.getKey())) {
            continue;
        }
        if (soulmatesOrdered.containsKey(entry.getValue()) || soulmatesOrdered.containsValue(
            entry.getValue())) {
            continue;
        }
  ...
  public ServerPlayerEntity getSoulmate(UUID playerUUID) {
      if (!isSoulmateOnline(playerUUID)) {
          return null;
      }
    UUID soulmateUUID = soulmates.get(playerUUID);
  ...
  public UUID getSoulmateUUID(UUID playerUUID) {
      if (playerUUID == null || !soulmates.containsKey(playerUUID)) {
          return null;
      }
  ...
  public void rollSoulmates() {
    ...
    PlayerUtils.sendTitleToPlayers(playersToRoll, Text.literal("3").formatted(Formatting.GREEN), 5,
        20, 5);
    TaskScheduler.scheduleTask(25, () -> {
      PlayerUtils.playSoundToPlayers(playersToRoll, SoundEvents.UI_BUTTON_CLICK.value());
      PlayerUtils.sendTitleToPlayers(playersToRoll, Text.literal("2").formatted(Formatting.GREEN),
          5, 20, 5);
    });
    TaskScheduler.scheduleTask(50, () -> {
      PlayerUtils.playSoundToPlayers(playersToRoll, SoundEvents.UI_BUTTON_CLICK.value());
      PlayerUtils.sendTitleToPlayers(playersToRoll, Text.literal("1").formatted(Formatting.GREEN),
          5, 20, 5);
    });
    TaskScheduler.scheduleTask(75, () -> {
      PlayerUtils.sendTitleToPlayers(playersToRoll,
          Text.literal("Your soulmate is...").formatted(Formatting.GREEN), 10, 50, 20);
      PlayerUtils.playSoundToPlayers(playersToRoll,
          SoundEvent.of(Identifier.of("minecraft", "doublelife_soulmate_wait")));
    });
    TaskScheduler.scheduleTask(165, () -> {
      ...
      for (ServerPlayerEntity player : playersToRoll) {
        ...
        PlayerUtils.playSoundToPlayer(player,
            SoundEvent.of(Identifier.of("minecraft", "doublelife_soulmate_chosen")));
      }
    });
  }
  ...
  public void distributePlayers() {
    ...
    OtherUtils.executeCommand(
        TextUtils.formatString("spreadplayers {} {} 0 {} false @a[tag=randomTeleport]",
            border.getCenterX(), border.getCenterZ(), (border.getSize() / 2)));
  ...
  public void chooseRandomSoulmates() {
    ...
    if (playersToRoll.size() % 2 != 0) {
      ...
      PlayerUtils.broadcastMessageToAdmins(
          Text.literal(" [DoubleLife] ").append(remove.getStyledDisplayName()).append(
              " was not paired with anyone, as there is an odd number of non-assigned players online."));
    }
  ...
  public void onPlayerDamage(ServerPlayerEntity player, DamageSource source, float amount,
      CallbackInfo ci) {
      if (source.getType().msgId().equalsIgnoreCase("soulmate")) {
          return;
      }
      if (amount == 0) {
          return;
  ...
  public void onPlayerDeath(ServerPlayerEntity player, DamageSource source) {
    ...
    if (SOULBOUND_INVENTORIES && server != null && !server.getGameRules()
  ...
  public void syncPlayers(ServerPlayerEntity player, ServerPlayerEntity soulmate) {
      ...
      if (player.isDead() || soulmate.isDead()) {
          return;
      }
  ...
  public void canFoodHeal(ServerPlayerEntity player, CallbackInfoReturnable<Boolean> cir) {
    ...
    boolean canHealWithSaturationOther = soulmate.getHungerManager().getSaturationLevel() > 2.0F
        && soulmate.getHungerManager().getFoodLevel() >= 20;
  ...
  public void updateFood(ServerPlayerEntity player, ServerPlayerEntity soulmate) {
    ...
    IHungerManager hungerManager2 = (IHungerManager) soulmate.getHungerManager();
      if (hungerManager1 == null || hungerManager2 == null) {
          return;
      }

    int foodLevel = Math.min(hungerManager1.ls$getFoodLevel(), hungerManager2.ls$getFoodLevel());
    float saturation = Math.max(hungerManager1.ls$getSaturationLevel(),
        hungerManager2.ls$getSaturationLevel());
  ...
  public void setHungerManager(IHungerManager hungerManager1, IHungerManager hungerManager2,
  ...
  public void syncPlayerInventory(ServerPlayerEntity player, ServerPlayerEntity soulmate) {
      if (!SOULBOUND_INVENTORIES) {
          return;
      }
      if (isRecentlyDead(player) && isRecentlyDead(soulmate)) {
          return;
      }
    boolean swapDirection = isRecentlyDead(player) && !isRecentlyDead(soulmate);
  ...
  public void setPlayerInventory(ServerPlayerEntity player, PlayerInventory inventory) {
    ...
    for (int i = 0; i < Math.min(newInventory.size(), playerInventory.size()); i++) {
      ItemStack newStack = newInventory.get(i).copy();
        if (ItemStack.areEqual(playerInventory.getStack(i), newStack)) {
            continue;
        }
  ...
  public void syncStatusEffectsFrom(ServerPlayerEntity player, StatusEffectInstance effect,
      boolean add) {
    TaskScheduler.scheduleTask(0, () -> delayedSyncStatusEffectsFrom(player, effect, add));
  }

  public void delayedSyncStatusEffectsFrom(ServerPlayerEntity player, StatusEffectInstance effect,
      boolean add) {
      if (!SOULBOUND_EFFECTS) {
          return;
      }
    ServerPlayerEntity soulmate = getSoulmate(player);
      if (soulmate == null) {
      ...
      if (hasSoulmate(player1) && hasSoulmate(player2)) {
        if (getSoulmate(player1) == player2) {
          ...
          TaskScheduler.scheduleTask(200, () -> {
            PlayerUtils.sendTitleWithSubtitleToPlayers(allPlayers, Text.empty(),
                Text.of("§aYour fate is your own..."), 20, 40, 20);
          });
          TaskScheduler.scheduleTask(300, () -> {
            PlayerUtils.sendTitleWithSubtitleToPlayers(allPlayers, Text.empty(),
                Text.of("§cThere can only be one winner."), 20, 40, 20);
          });
