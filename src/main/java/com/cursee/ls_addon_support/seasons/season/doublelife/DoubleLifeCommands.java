...
public class DoubleLifeCommands {
        ...
        literal("soulmate")
            ...
            .then(literal("get")
                .then(argument("player", EntityArgumentType.player())
                    .executes(context -> getSoulmate(context.getSource(),
            ...
            .then(literal("reset")
                .then(argument("player", EntityArgumentType.players())
                    .executes(context -> resetSoulmate(context.getSource(),
  ...
  public static int setSoulmate(ServerCommandSource source, ServerPlayerEntity player,
      ServerPlayerEntity soulmate) {
    ...
    OtherUtils.sendCommandFeedback(source,
        TextUtils.format("{}'s soulmate is now {}", player, soulmate));
    return 1;
  }
  ...
  public static int getSoulmate(ServerCommandSource source, ServerPlayerEntity player) {
    ...
    OtherUtils.sendCommandFeedback(source,
        TextUtils.format("{}'s soulmate is {}", player, soulmate));
    return 1;
  }

  public static int resetSoulmate(ServerCommandSource source,
    ...
    if (affected.size() == 1) {
      OtherUtils.sendCommandFeedback(source,
          TextUtils.format("{}'s soulmate was reset", affected.getFirst()));
      return 1;
    }
    OtherUtils.sendCommandFeedback(source,
        TextUtils.format("Soulmate was reset for {} targets", affected.size()));
    return 1;
  }
  ...
  public static int listSoulmates(ServerCommandSource source) {
    ...
    for (Map.Entry<UUID, UUID> entry : season.soulmatesOrdered.entrySet()) {
      ...
      OtherUtils.sendCommandFeedbackQuiet(source,
          TextUtils.format("{}'s soulmate is {}", text1, text2));
    }
