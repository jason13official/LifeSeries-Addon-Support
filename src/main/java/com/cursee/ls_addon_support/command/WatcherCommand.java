...
public class WatcherCommand {
  ...
  private static int info(ServerCommandSource source) {
    OtherUtils.sendCommandFeedbackQuiet(source, Text.of(
        "§7Watchers are players that are online, but are not affected by most season mechanics. They can only observe."));
    OtherUtils.sendCommandFeedbackQuiet(source,
        Text.of("§7This is very useful for spectators and for admins."));
    OtherUtils.sendCommandFeedbackQuiet(source,
  ...
  private static int listWatchers(ServerCommandSource source) {
    ...
    OtherUtils.sendCommandFeedbackQuiet(source,
        TextUtils.formatLoosely("Current Watchers: §7{}", WatcherManager.getWatchers()));
    return 1;
  }

  private static int addWatchers(ServerCommandSource source,
    ...
    if (targets.size() == 1) {
      OtherUtils.sendCommandFeedback(source,
          TextUtils.format("{} is now a Watcher", targets.iterator().next()));
    } else {
      OtherUtils.sendCommandFeedback(source,
          TextUtils.format("{} targets are now Watchers", targets.size()));
    }

    return 1;
  }

  private static int removeWatchers(ServerCommandSource source,
      Collection<ServerPlayerEntity> targets) {
      if (targets == null || targets.isEmpty()) {
          return -1;
      }
    ...
    if (targets.size() == 1) {
      OtherUtils.sendCommandFeedback(source,
          TextUtils.format("{} is no longer a Watcher", targets.iterator().next()));
    } else {
      OtherUtils.sendCommandFeedback(source,
          TextUtils.format("{} targets are no longer Watchers", targets.size()));
    }
