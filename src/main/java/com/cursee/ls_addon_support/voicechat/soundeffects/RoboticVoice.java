package com.cursee.ls_addon_support.voicechat.soundeffects;
...
public class RoboticVoice {
  ...
  private static final long MAX_HISTORY_AGE_MS = 300;
  ...
  public static short[] applyEffect(UUID uuid, short[] input) {
    ...
    if (history.isEmpty()) {
      return new short[input.length]; // Silence if no recent stats
    }
  ...
  private static short[] synthHarmonicTone(UUID uuid, int length, double baseFreq,
      double amplitude) {
