...
public class ItemStackUtils {
  ...
  public static void setCustomComponentBoolean(ItemStack itemStack, String componentKey,
      boolean value) {
      if (itemStack == null) {
          return;
      }
  ...
  public static void setCustomComponentString(ItemStack itemStack, String componentKey,
      String value) {
      if (itemStack == null) {
          return;
      }
  ...
  public static String getCustomComponentString(ItemStack itemStack, String componentKey) {
    ...
    NbtCompound nbtComp = nbtComponent.copyNbt();
      if (!nbtComp.contains(componentKey)) {
          return null;
      }
  ...
  public static void removeCustomComponentEntry(ItemStack itemStack, String componentEntry) {
    NbtComponent nbt = itemStack.getComponents().get(DataComponentTypes.CUSTOM_DATA);
      ...
      if (!nbt.contains(componentEntry)) {
          return;
      }
    ...
    if (nbtComp.isEmpty()) {
      itemStack.set(DataComponentTypes.CUSTOM_DATA,
          itemStack.getDefaultComponents().get(DataComponentTypes.CUSTOM_DATA));
    } else {
      itemStack.set(DataComponentTypes.CUSTOM_DATA, NbtComponent.of(nbtComp));
    }
  }
  ...
  public static void spawnItemForPlayer(ServerWorld world, Vec3d position, ItemStack stack,
      PlayerEntity player) {
    ...
    itemEntity.setVelocity(itemEntity.getVelocity().getX() / 4, 0.2,
        itemEntity.getVelocity().getZ() / 4);
      if (player != null) {
  ...
  public static void spawnItemForPlayerWithVelocity(ServerWorld world, Vec3d position,
      ItemStack stack, PlayerEntity player, Vec3d velocity) {
    if (world == null || stack.isEmpty()) {
      return;
    }
    ItemEntity itemEntity = new ItemEntity(world, position.x, position.y, position.z, stack);
  ...
  @Nullable
  public static RegistryEntry<Enchantment> getEnchantmentEntry(
