...
public abstract class DefaultScreen extends Screen {
  ...
  private static final Identifier BACKGROUND_TEXTURE_LEFT = Identifier.of("lifeseries",
      "textures/gui/gui_left.png");
  private static final Identifier BACKGROUND_TEXTURE_RIGHT = Identifier.of("lifeseries",
      "textures/gui/gui_right.png");
  ...
  public void renderBackground(DrawContext context, int mouseX, int mouseY) {
    //? if <= 1.21 {
    if (!isScaled()) {
      RenderUtils.drawTexture(context, BACKGROUND_TEXTURE_LEFT, startX, startY, 0, 0, BG_WIDTH / 2,
          BG_HEIGHT);
      RenderUtils.drawTexture(context, BACKGROUND_TEXTURE_RIGHT, startX + BG_WIDTH / 2, startY, 0,
          0, BG_WIDTH / 2, BG_HEIGHT);
    } else {
      RenderUtils.drawTextureScaled(context, BACKGROUND_TEXTURE_LEFT, startX, startY, 0, 0,
          BG_WIDTH_UNSCALED / 2, BG_HEIGHT_UNSCALED, scaleX, scaleY);
      RenderUtils.drawTextureScaled(context, BACKGROUND_TEXTURE_RIGHT, startX + BG_WIDTH / 2,
          startY, 0, 0, BG_WIDTH_UNSCALED / 2, BG_HEIGHT_UNSCALED, scaleX, scaleY);
    }
    //?} else {
        /*if (!isScaled()) {
  ...
  public void renderClose(DrawContext context, int mouseX, int mouseY) {
      if (isInCloseRegion(mouseX, mouseY)) {
          RenderUtils.drawTextRight(context, textRenderer, Text.of("§l✖"), endX - 4, startY + 4);
      } else {
          RenderUtils.drawTextRight(context, textRenderer, Text.of("✖"), endX - 4, startY + 4);
      }
  }
