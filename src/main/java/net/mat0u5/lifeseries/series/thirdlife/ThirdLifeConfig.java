...
public class ThirdLifeConfig extends ConfigManager {
    ...
    /*
    {
        index += NetworkHandlerServer.sendConfig(player, "integer", "default_lives", 0, "Default Lives", "The number of lives every player will have by default.", List.of(String.valueOf(getOrCreateInt("default_lives", 3)), "3"));
        index += NetworkHandlerServer.sendConfig(player, "integer", "max_player_health", 1, "Default Health", "The amount of health (half-hearts) every player will have by default.", List.of(String.valueOf(getOrCreateInt("max_player_health", 20)), "20"));
        index += NetworkHandlerServer.sendConfig(player, "string", "blacklist_items", 2, "Blacklisted Items", "List of banned items.", List.of(getOrCreateProperty("blacklist_items","["+String.join(", ", BLACKLISTED_ITEMS)+"]"), "["+String.join(", ", BLACKLISTED_ITEMS)+"]"));
        index += NetworkHandlerServer.sendConfig(player, "string", "blacklist_blocks", 3, "Blacklisted Blocks", "List of banned blocks.", List.of(getOrCreateProperty("blacklist_blocks","["+String.join(", ", BLACKLISTED_BLOCKS)+"]"), "["+String.join(", ", BLACKLISTED_BLOCKS)+"]"));
        index += NetworkHandlerServer.sendConfig(player, "string", "blacklist_clamped_enchants", 5, "Clamped Enchants", "List of enchantments clamped to level 1 (any higher levels will be set to lvl1).", List.of(getOrCreateProperty("blacklist_clamped_enchants","["+String.join(", ", CLAMPED_ENCHANTMENTS)+"]"), "["+String.join(", ", CLAMPED_ENCHANTMENTS)+"]"));
        index += NetworkHandlerServer.sendConfig(player, "boolean", "custom_enchanter_algorithm", 7, "Custom Enchanter Algorithm", "Modifies the enchanting table algorithm to allow players to get all enchants even without bookshelves.", List.of(String.valueOf(getOrCreateBoolean("custom_enchanter_algorithm", true)), "true"));
        index += NetworkHandlerServer.sendConfig(player, "boolean", "spawn_egg_allow_on_spawner", 10, "Spawn Egg Allow on Spawners", "Controls whether players should be able to use the spawn eggs on spawners.", List.of(String.valueOf(getOrCreateBoolean("spawn_egg_allow_on_spawner", false)), "false"));
        index += NetworkHandlerServer.sendConfig(player, "boolean", "spawner_recipe", 11, "Spawner Recipe", "Controls whether the spawner crafting recipe is enabled.", List.of(String.valueOf(getOrCreateBoolean("spawner_recipe", false)), "false"));
